...
function bindSubmit() {
    $('#registsubmit').click(function () {
        ...
        if (result) {
            ...
            var oauthId = getQueryString("oauthId");
            var name = getQueryString("name");
            var openId = getQueryString("openId");
            var unionid = getQueryString("unionid");
            var unionopenid = getQueryString("unionopenid");
            ...
            $.post('/Register/RegisterUser', par, function (data) {
                loading.close();
                if (data.success) {
                    var strMessage = "注册成功！";
                    if (data.num > 0) {
                    ...
                    $.cookie('Himall-DefaultUserID', data.memberId, { path: "/", expires: 365 });
                    $.dialog.succeedTips(strMessage, function () {
                        var returnurl = getQueryString("returnurl");
                        //alert(returnurl);
                        if (returnurl != null && returnurl!=NaN&&returnurl != "" && returnurl.indexOf("http")>=0) {
...
function checkValid() {
    ...
    result = result & checkRepeatPasswordIsValid();
    result = result & checkMobileIsValid(false) & checkEmailIsValid(false);
...
function sendmCode() {
    ...
    var checkCode = $('#checkCode').val();
    if (checkCode == "") {
        errorLabel.html('验证码不能为空').show();
    }
    $("#sendMobileCode").attr("disabled", "disabled");
    jQuery.ajax({
        type: "post",
        url: "/Register/SendCode?pluginId=Himall.Plugin.Message.SMS&destination=" + $("#cellPhone").val() + "&code=" + checkCode,
        success: function (result) {
            if (result.success == true) {
                ...
                $('#cellPhone_error').html(result.msg).show();
...
function checkEmailIsValid(isasync) {
